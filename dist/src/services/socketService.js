...
exports.notifyNewChat = notifyNewChat;
exports.notifyNewMessage = notifyNewMessage;
exports.notifyChatsUpdated = notifyChatsUpdated;
exports.notifyMessagesRead = notifyMessagesRead;
exports.notifyMessageStatus = notifyMessageStatus;
exports.notifyChatDeleted = notifyChatDeleted;
...
function initializeSocketIO(httpServer) {
    ...
    io.on('connection', (socket) => {
        const { userId, organizationId, userEmail } = socket.data;
        ...
        socket.on('unsubscribe:chat', (chatId) => {
            const chatRoom = `chat_${chatId}`;
 ...
 */
function notifyNewChat(organizationId, chatData) {
    try {
        const io = getSocketIO();
        const room = `org_${organizationId}`;
        io.to(room).emit('chat:new', Object.assign(Object.assign({}, chatData), { timestamp: new Date().toISOString() }));
 ...
 */
function notifyNewMessage(organizationId, messageData) {
    try {
        const io = getSocketIO();
        const orgRoom = `org_${organizationId}`;
        const chatRoom = `chat_${messageData.chatId}`;
        const eventData = Object.assign(Object.assign({}, messageData), { timestamp: messageData.timestamp || new Date().toISOString() });
 ...
 */
function notifyChatsUpdated(organizationId, chatData) {
    try {
        const io = getSocketIO();
        const orgRoom = `org_${organizationId}`;
        const chatRoom = `chat_${chatData.id}`;
        const eventData = Object.assign(Object.assign({}, chatData), { timestamp: new Date().toISOString() });
 ...
 */
function notifyMessagesRead(organizationId, chatId, readByUserId) {
    try {
        const io = getSocketIO();
        const orgRoom = `org_${organizationId}`;
        const chatRoom = `chat_${chatId}`;
 ...
 */
function notifyMessageStatus(organizationId, messageId, status, chatId) {
    try {
        const io = getSocketIO();
        const orgRoom = `org_${organizationId}`;
        const chatRoom = `chat_${chatId}`;
 ...
 */
function notifyChatDeleted(organizationId, chatId) {
    try {
        const io = getSocketIO();
        const orgRoom = `org_${organizationId}`;
        const chatRoom = `chat_${chatId}`;
...
function notifyUser(userId, event, data) {
    try {
        const io = getSocketIO();
        const userRoom = `user_${userId}`;
        io.to(userRoom).emit(event, Object.assign(Object.assign({}, data), { timestamp: new Date().toISOString() }));
...
function broadcastToOrganization(organizationId, event, data) {
    try {
        const io = getSocketIO();
        const orgRoom = `org_${organizationId}`;
        io.to(orgRoom).emit(event, Object.assign(Object.assign({}, data), { timestamp: new Date().toISOString() }));
