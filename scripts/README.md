# 📜 Административные скрипты

Коллекция утилит для управления данными messenger-backend.

## 📁 Список скриптов

### 🗑️ Очистка данных

| Скрипт | Описание | Безопасность | Рекомендация |
|--------|----------|--------------|--------------|
| `clear-all-data.sql` | SQL скрипт для быстрого удаления | ⚠️ Нет бэкапа | Только для разработки |
| `clear-all-chats-messages.js` | Node.js скрипт с dry-run | ✅ Dry-run режим | Для тестирования |
| `clear-all-chats-messages-with-backup.js` | Node.js с автобэкапом | ✅✅ Полный бэкап | Для production |

### 🔧 Миграции

| Скрипт | Описание |
|--------|----------|
| `migrate-ticket-numbers.js` | Миграция нумерации тикетов |
| `test-ticket-reopen.js` | Тест переоткрытия тикетов |
| `clean-app-state.js` | Очистка app-state-sync-key |

### 💬 WABA (WhatsApp Business)

| Скрипт | Описание |
|--------|----------|
| `waba-broadcast-template.js` | Рассылка шаблонного сообщения по списку номеров через Graph API |

---

## 🚀 Быстрый старт

### Вариант 1: SQL (самый быстрый)

```bash
# Создать бэкап
pg_dump -U postgres messenger_db > backup.sql

# Удалить все данные
psql -U postgres -d messenger_db -f scripts/clear-all-data.sql
```

### Вариант 2: Node.js без бэкапа

```bash
# Dry-run (просмотр без удаления)
node scripts/clear-all-chats-messages.js --dry-run

# Реальное удаление
node scripts/clear-all-chats-messages.js
```

### Вариант 3: Node.js с автобэкапом (рекомендуется)

```bash
# Удаление с автоматическим созданием бэкапа
node scripts/clear-all-chats-messages-with-backup.js

# Указать путь к бэкапу
node scripts/clear-all-chats-messages-with-backup.js --backup-dir /path/to/backups
```

### WABA: рассылка шаблона по списку

```bash
# Пример списка получателей
cat scripts/waba-broadcast-numbers.example.txt

# Отправка шаблона new_number (ru) по списку
WABA_ACCESS_TOKEN="<your_token>" \
node scripts/waba-broadcast-template.js \
   --list scripts/waba-broadcast-numbers.example.txt \
   --phone-number-id 958088394044701 \
   --template new_number \
   --language ru

# Dry-run (без отправки)
node scripts/waba-broadcast-template.js \
   --list scripts/waba-broadcast-numbers.example.txt \
   --phone-number-id 958088394044701 \
   --dry-run
```

---

## 📖 Подробная документация

Полная документация доступна в:
- [`CLEAR_DATA_GUIDE.md`](../CLEAR_DATA_GUIDE.md) - Руководство по очистке данных

---

## ⚠️ Предупреждения

### Перед использованием скриптов:

1. **Создайте резервную копию БД**
   ```bash
   pg_dump -U postgres messenger_db > backup-$(date +%Y%m%d-%H%M%S).sql
   ```

2. **Проверьте переменные окружения**
   ```bash
   echo $DATABASE_URL
   ```

3. **Используйте dry-run режим**
   ```bash
   node scripts/clear-all-chats-messages.js --dry-run
   ```

4. **Не используйте на production без бэкапа**

---

## 🔐 Безопасность

### Уровни безопасности скриптов:

| Уровень | Скрипт | Защита |
|---------|--------|--------|
| 🔴 **Низкий** | `clear-all-data.sql` | Только 5-секундная задержка |
| 🟡 **Средний** | `clear-all-chats-messages.js` | Dry-run + интерактивное подтверждение |
| 🟢 **Высокий** | `clear-all-chats-messages-with-backup.js` | Автобэкап + dry-run + подтверждение |

### Рекомендации:

- **Разработка**: Любой скрипт
- **Staging**: `clear-all-chats-messages.js` с dry-run
- **Production**: ТОЛЬКО `clear-all-chats-messages-with-backup.js`

---

## 🛠️ Устранение проблем

### Проблема: "Ошибка подключения к БД"

```bash
# Проверьте DATABASE_URL
echo $DATABASE_URL

# Установите правильное значение
export DATABASE_URL="postgresql://user:password@localhost:5432/messenger_db"
```

### Проблема: "pg_dump not found"

```bash
# macOS
brew install postgresql

# Ubuntu/Debian
sudo apt-get install postgresql-client
```

### Проблема: "Недостаточно прав"

```bash
# Проверьте права пользователя PostgreSQL
psql -U postgres -c "\du"

# Предоставьте права
psql -U postgres -c "GRANT ALL ON DATABASE messenger_db TO your_user;"
```

---

## 📞 Поддержка

При возникновении проблем:

1. Проверьте логи: `tail -f logs/app.log`
2. Проверьте состояние БД: `psql -U postgres messenger_db -c "\dt"`
3. Обратитесь к документации: `CLEAR_DATA_GUIDE.md`

---

**Создано**: 2025
**Версия**: 1.0.0
